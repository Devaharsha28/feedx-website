...
export default function InstituteAdmin() {
    ...
    const fetchInstitute = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          toast({ title: "Institute not found", variant: "destructive" });
          navigate("/admin");
        }
      } catch (error) {
        console.error("Failed to fetch institute:", error);
        toast({ title: "Failed to load institute", variant: "destructive" });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const saveInstitute = async () => {
    ...
    try {
      ...
      toast({ title: "Failed to save institute", variant: "destructive" });
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <main className="flex-1 container mx-auto px-4 py-24">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-foreground">{institute.name}</h1>
              <p className="text-muted-foreground">Code: {institute.code}</p>
            </div>
          </div>
          <Button onClick={saveInstitute} disabled={isSaving}>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid grid-cols-5 lg:grid-cols-9 gap-1 mb-8 bg-card/50 p-1 rounded-lg">
            ...
            <TabsTrigger value="departments" className="flex items-center gap-1 text-xs">
              <Building2 className="w-3 h-3" /> Depts
            </TabsTrigger>
            <TabsTrigger value="faculty" className="flex items-center gap-1 text-xs">
            ...
            <TabsTrigger value="placements" className="flex items-center gap-1 text-xs">
              <Briefcase className="w-3 h-3" /> Place
            </TabsTrigger>
            <TabsTrigger value="gallery" className="flex items-center gap-1 text-xs">
              <Image className="w-3 h-3" /> Gallery
            </TabsTrigger>
            <TabsTrigger value="infrastructure" className="flex items-center gap-1 text-xs">
              <Building className="w-3 h-3" /> Infra
            </TabsTrigger>
            <TabsTrigger value="reviews" className="flex items-center gap-1 text-xs">
              <MessageSquare className="w-3 h-3" /> Reviews
            </TabsTrigger>
            <TabsTrigger value="qna" className="flex items-center gap-1 text-xs">
    ...
    <div className="grid gap-6">
      <Card className="glass-card">
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          <div>
            ...
            <Input value={institute.name} onChange={(e) => updateField('name', e.target.value)} />
          </div>
          <div>
            <Label>Code</Label>
          ...
          <div>
            <Label>Place</Label>
            <Input value={institute.place || ''} onChange={(e) => updateField('place', e.target.value)} />
          </div>
          <div>
            <Label>District</Label>
            <Input value={institute.dist || ''} onChange={(e) => updateField('dist', e.target.value)} />
          </div>
          <div>
            <Label>Principal Name</Label>
            <Input value={institute.principal || ''} onChange={(e) => updateField('principal', e.target.value)} />
          </div>
          <div>
            <Label>Established Year</Label>
            <Input value={institute.established || ''} onChange={(e) => updateField('established', e.target.value)} />
          </div>
      ...
      <Card className="glass-card">
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          <div className="md:col-span-2">
            <Label>Address</Label>
            <Textarea value={institute.address || ''} onChange={(e) => updateField('address', e.target.value)} rows={2} />
          </div>
          <div>
            <Label>Phone</Label>
            <Input value={institute.phone || ''} onChange={(e) => updateField('phone', e.target.value)} />
          </div>
          <div>
            <Label>Email</Label>
            <Input type="email" value={institute.email || ''} onChange={(e) => updateField('email', e.target.value)} />
          </div>
          <div>
            <Label>Website</Label>
            <Input value={institute.website || ''} onChange={(e) => updateField('website', e.target.value)} />
          </div>
        </CardContent>
      </Card>
    ...
    <div className="grid gap-6">
      <Card className="glass-card">
        ...
        <CardContent className="grid gap-4 md:grid-cols-4">
          ...
          <Input placeholder="Code (e.g., CSE)" value={newDept.code} onChange={(e) => setNewDept({...newDept, code: e.target.value})} />
          <Input placeholder="Intake" type="number" value={newDept.intake} onChange={(e) => setNewDept({...newDept, intake: e.target.value})} />
  ...
  return (
    <div className="grid gap-6">
      <Card className="glass-card">
        ...
        <CardContent className="grid gap-4 md:grid-cols-4">
          <Input placeholder="Name (e.g., Computer Lab)" value={newInfra.name} onChange={(e) => setNewInfra({...newInfra, name: e.target.value})} />
          <Input placeholder="Capacity (e.g., 60 students)" value={newInfra.capacity} onChange={(e) => setNewInfra({...newInfra, capacity: e.target.value})} />
          <Input placeholder="Description" value={newInfra.description} onChange={(e) => setNewInfra({...newInfra, description: e.target.value})} />
        ...
        {infrastructure.map((infra) => (
          <Card key={infra.id} className="glass-card">
            <CardContent className="pt-6">
              ...
              <Input type="file" accept="image/*" onChange={(e) => handleImageUpload(e, infra.id)} disabled={isUploading} />
            </CardContent>
          </Card>
        ))}
  ...
  return (
    <div className="grid gap-6">
      <Card className="glass-card">
        ...
        <CardContent className="grid gap-4 md:grid-cols-3">
          <Input placeholder="Reviewer Name" value={newReview.name} onChange={(e) => setNewReview({...newReview, name: e.target.value})} />
          <select 
            className="px-3 py-2 rounded-md border bg-background"
    ...
    <div className="grid gap-6">
      <Card className="glass-card">
        ...
        <CardContent className="grid gap-4">
          <Input placeholder="Asked by (optional)" value={newQA.askedBy} onChange={(e) => setNewQA({...newQA, askedBy: e.target.value})} />
          <Textarea placeholder="Question" value={newQA.question} onChange={(e) => setNewQA({...newQA, question: e.target.value})} />
          <Textarea placeholder="Answer" value={newQA.answer} onChange={(e) => setNewQA({...newQA, answer: e.target.value})} />
