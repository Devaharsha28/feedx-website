...
const normalizePin = (p: string) => (p || '').trim();
  ...
  const fetchAll = async () => {
    ...
    try {
      const [resultsResp, attendanceResp] = await Promise.allSettled([
        fetch(`${API_BASE}/api/results?pin=${encodeURIComponent(trimmed)}`),
        fetch(`${API_BASE}/api/attendance?pin=${encodeURIComponent(trimmed)}`),
      ]);
      ...
      if (attendanceResp.status === 'fulfilled') {
        ...
        if (!attRes.ok) {
          ...
          if (!attJson?.success) {
            ...
            setAttendance((attJson.attendanceSummary || null) as AttendanceSummary | null);
          }
        }
      } else {
      ...
      // Only show a blocking error if results failed; attendance can be partial.
      if (resultsError) {
                  ...
                  {results.weakSubjects.length ? (
                    <ul className="list-disc pl-5 space-y-1">
                      {results.weakSubjects.map((s) => (
                        <li key={`${s.subject}-${s.grade}`}>{s.subject} — {s.marks ?? '—'} ({s.grade})</li>
                      ))}
                    </ul>
                  ) : (
                    '—'
